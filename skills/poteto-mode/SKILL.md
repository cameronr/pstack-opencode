...
**Defaults for every `Agent` call.** `run_in_background: true`, file pointers not inlined context, explicit subagent type per role (configurable via `/setup-pstack`; defaults `poteto-agent` for code, `general-purpose` for prose and judgment, `code-reviewer` for review, `Explore` for read-only exploration). ZCode runs every subagent on the session model, so diversity comes from the subagent type and the prompt, not the model family. Code delegates tier by role: the hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your judgment role, precisely specified sequences of steps go to your executor role, trivial mechanical edits go to the fast path. Per-role lines in `~/.zcode/pstack-roles.md` (written by `/setup-pstack`) override these defaults and the type choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`); a role with no line keeps its default.
