...
2. Pick the wake mechanism using ZCode's scheduled automations (the Cron tools; a built-in, not a pstack skill). Register a recurring automation whose prompt re-states the predicate and tells the next run where the trail lives; delete the automation when the predicate is met. An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
