...
This skill orchestrates three others: an inline mining pass (see step 1), the `skill-creator` skill (from the `skill-creator` plugin) (authoring), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
- the `skill-creator` skill (from the `skill-creator` plugin): skill authoring process and writing guidelines.
