...
copies `skills/*` into `~/.config/opencode/skills/` and `agents/*.md` into `~/.config/opencode/agents/`. same-named entries are replaced; anything else in those directories is untouched. safe to re-run; it prunes items removed from the repo (tracked in a manifest) and leaves your own skills and agents alone.

alternatives:

```sh
./install.sh --link       # symlink instead of copy; repo edits show up without reinstalling
./install.sh --prefix DIR # install into DIR instead of ~/.config/opencode
```

symlinks are absolute: moving the repo breaks them. re-run the installer from the new location.

**updating:** re-run the installer after `git pull`. with `--link`, `git pull` alone suffices.

opencode2 (beta) can skip the copy for skills and point its config at the repo:
...
that entry covers skills only; the agents still need the installer (or a manual copy of `agents/*.md` into `~/.config/opencode/agents/`). use either this or the installer, not both.
...
[`/poteto-mode`](./skills/poteto-mode/SKILL.md) works extremely well with an OS-level wake (a cron job or launchd agent running `opencode run` headless). you can make the agent work for many hours without sacrificing rigor.
...
| [`/setup-pstack`](./skills/setup-pstack/SKILL.md) | you want to pick which subagent types pstack uses per role. detects your available types and writes a config file; can also pin per-role models in `opencode.json`. |
...
pstack also ships a subagent that runs my style end to end. spawn it from a parent agent via [`subagent_type: "poteto-agent"`](./agents/poteto-agent.md). it reads `poteto-mode` in full, including its inline principles index, before doing any work. substituting `general` skips that read and drifts.
...
this port adds two more: [code-reviewer](./agents/code-reviewer.md) (`subagent_type: "code-reviewer"`), a correctness-first diff reviewer that reports and never fixes, and [code-architect](./agents/code-architect.md) (`subagent_type: "code-architect"`), an architecture critic that proposes and never edits. the review panels fan out across them.
...
- the `skill-creator` skill (used by `reflect`, `automate-me`, and `authoring-a-skill` for authoring SKILL.md files) is not part of this repo; install it separately if you use those flows.
- browser / web UI control comes from the `browser-use` plugin's `control-browser` skill when available; for CLIs and TUIs, run them in a shell and read the real output.

install those alongside pstack if you want the full set.
...
roles are configurable too. type [`/setup-pstack`](./skills/setup-pstack/SKILL.md). it detects the subagent types available in your session and writes a small file, `~/.config/opencode/pstack-roles.md`, mapping each role (code, judgment, the review panels) to a type. every skill reads it and falls back to sensible defaults when the file is absent, so you override only what you want. it can also set per-role models in `opencode.json` if you want different models per role.
...
opencode has no scheduled-automation equivalent today, so the pack is reference material in this port. the natural future is a system cron entry (or launchd agent) that calls `opencode run` with benny's prompts.
...
this repo is a port of [backnotprop/pstack](https://github.com/backnotprop/pstack), the original standalone repo, built off of the [ZCode port](https://github.com/Luks3110/pstack-zcode). the port applied a mechanical convention mapping (tool names, subagent types, paths, dual frontmatter) plus one-off semantic hand edits (transcript rewrites, cloud-agent -> background-subagent rewrites, model -> subagent-type wording). `scripts/check-upstream.py` fetches upstream, applies the mechanical mapping, and diffs the result against this repo, so the report shows only what upstream changed that we have not absorbed:
