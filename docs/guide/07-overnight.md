...
- "loop until done" asks for an OS-level wake: opencode has no built-in scheduler, so a cron job or launchd agent runs `opencode run` (headless) against the session or repo. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) registers one to re-check the finish condition on a heartbeat and deletes it when the predicate passes.
