...
4. **Spawn N parallel candidates** on different subagent types per the **arena** skill's Phase B. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded judge** on a different subagent type per the **arena** skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a subagent type name.
6. **Verify the chain from transcripts, not self-report.** Read each candidate's session rows from the workspace's sessions in `~/.local/share/opencode/opencode.db` (query with `sqlite3` in read-only mode, filtering `session.directory` / `session_v2.directory` on the workspace path). Do not read other projects' sessions; that crosses workspace boundaries and reads private chats from unrelated projects. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
7. **Read every candidate output yourself** end to end. Compare to the judge's verdict. Disagreement means the judge is biased or the rubric is ambiguous. Synthesize.
