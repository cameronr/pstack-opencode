...
Transcripts live under `~/.zcode/cli/` (session rollout files), one JSONL per session. Every line is one chat message. Locate the current workspace's sessions there; never read another project's transcripts without being asked.
