...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs the [`/unslop`](../../skills/unslop/SKILL.md) skill on the diff before each commit and applies it again to the PR description and commit bodies. The diff pass is the code-side cleanup: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
The division of labor is worth keeping straight. `/unslop` cleans slop out of code diffs and prose, and `/no-comments` hands the comments to a reviewer who didn't write them.

**Pitfall:** cleanup is not optional polish. A diff with narrating comments and defensive dead weight reads as unfinished to reviewers, and the extra code is where the next bug hides. If the diff feels padded, say `unslop it` before you commit, not after review calls it out.
