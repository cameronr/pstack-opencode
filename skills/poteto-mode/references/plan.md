...
If a phase creates or edits a skill, the phase instructs the implementer to use the **skill-creator** skill (from the `skill-creator` plugin, for authoring SKILL.md files).
...
- Browser / Electron / Web UIs: the `control-browser` skill from the `browser-use` plugin.
- CLIs and TUIs: run them in a shell yourself and read the real output.
