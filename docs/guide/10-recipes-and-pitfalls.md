...
- **Expecting per-role models.** opencode runs every subagent on the session model unless its agent file or config sets one; the roles in [Setup](./01-setup.md) pick subagent types by default, and diversity comes from posture and prompt, not model family.
