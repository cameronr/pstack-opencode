...
3. Pick the runners. Use `arena runners` from `~/.config/opencode/pstack-roles.md` when present. Otherwise default to one each on `poteto-agent`, `code-reviewer`, `code-architect`, `general`. Spawn more when the arena covers multiple design directions. Repeat a type when the work is generation-bound rather than judgment-sensitive.
...
After all Phase B candidates complete, choose one review posture from the `arena cross-judge pool` in `~/.config/opencode/pstack-roles.md` when present. Otherwise use `code-reviewer`, `code-architect`, `general`. Prefer a type that did not produce any candidate. Spawn one readonly judge subagent (`subagent_type: "explore"`) with that posture. It sees the rubric and the candidates by path label, scores each criterion, and recommends a base with rationale. It runs in parallel with the parent's reading in Phase D, not with the candidates themselves. Spawning while candidates are still writing means the judge sees partial or empty outputs and reports them as dropouts.
