...
Every reply for a run that produced a trail ends with an "Attention" section. Lead with the reviewer's subagent type on its own line (`reviewed by <subagent type>`), then list each flag pointing to specific rows or moments. "No flags" is a valid value; the subagent type is not. The self-audit asks if the log told the truth; this asks what the user should still scrutinize even when it did.
