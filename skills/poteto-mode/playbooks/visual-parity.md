...
4. Verify each component against its baseline via image diff on the matching surface via the control skill (the `browser-use` plugin's `control-browser` for browser surfaces). A nonzero diff is a fail; investigate the pixel delta, don't wave it through. A scheduled automation (Cron) per component until the diff is zero.
